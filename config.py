...
def get_kis_env() -> str:
    value = os.getenv("KIS_ENV", "mock").strip().lower()
    return "real" if value == "real" else "mock"


def resolve_base_url() -> str:
    return os.getenv("KIS_BASE_URL", KIS_BASE_URLS[get_kis_env()]).rstrip("/")
...
class KISTokenManager:
    ...
    def _credentials_cache_key(self) -> str:
        raw = f"{self.base_url}:{self.credentials.app_key}"
...
def load_credentials() -> KISCredentials:
    ...
    kis_env = get_kis_env()
    if kis_env == "real":
        app_key = os.getenv("KIS_REAL_APP_KEY") or os.getenv("KIS_APP_KEY") or os.getenv("APP_KEY")
        app_secret = os.getenv("KIS_REAL_APP_SECRET") or os.getenv("KIS_APP_SECRET") or os.getenv("APP_SECRET")
    else:
        app_key = os.getenv("KIS_MOCK_APP_KEY") or os.getenv("KIS_APP_KEY") or os.getenv("APP_KEY")
        app_secret = os.getenv("KIS_MOCK_APP_SECRET") or os.getenv("KIS_APP_SECRET") or os.getenv("APP_SECRET")

    missing = [name for name, value in {"KIS app key": app_key, "KIS app secret": app_secret}.items() if not value]
    if missing:
...
def create_token_manager() -> KISTokenManager:
    return KISTokenManager(load_credentials(), base_url=resolve_base_url())
