import { buildKisIndices } from "../../server/kisDashboard";
...
export default async function handler(request: { method?: string }, response: ApiResponse) {
  ...
  try {
    writeJson(response, 200, await buildKisIndices());
  } catch (error) {
    const message = error instanceof Error ? error.message : "KIS index request failed.";
