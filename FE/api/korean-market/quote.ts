import { buildKisStockQuote } from "../../server/kisDashboard";
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    writeJson(response, 200, await buildKisStockQuote(symbol));
  } catch (error) {
    const message = error instanceof Error ? error.message : "KIS quote request failed.";
