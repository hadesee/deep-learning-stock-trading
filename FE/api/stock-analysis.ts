import { getStockAnalysisPayload } from "../server/pipelineResults";
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    writeJson(response, 200, await getStockAnalysisPayload(ticker));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Stock analysis request failed.";
