...
export function mergeMarketIndices(
  ...
  const liveBySymbol = new Map(indices.map((index) => [index.symbol, index]));
  const mergedSymbols = new Set<string>();
  const mergedIndices = data.indices.map((index) => {
    const live = liveBySymbol.get(index.symbol);
    if (!live) {
      return index;
    }

    mergedSymbols.add(index.symbol);
    return live;
  });

  for (const index of indices) {
    if (!mergedSymbols.has(index.symbol)) {
...
const INDEX_REQUEST_TIMEOUT_MS = 15000;
const QUOTE_REQUEST_TIMEOUT_MS = 15000;
...
export async function fetchStockQuoteData(ticker: string, signal?: AbortSignal): Promise<StockQuote | null> {
  const normalized = ticker.replace(/\D/g, "").padStart(6, "0").slice(-6);
  try {
    return await fetchJson<StockQuote>(
      `/api/korean-market/quote?symbol=${encodeURIComponent(normalized)}`,
      signal,
    ...
    if (import.meta.env.DEV) {
      console.warn(`KIS quote API unavailable for ${normalized}.`, error);
    }
    return null;
  }
}
...
export async function fetchMarketIndicesData(
  signal?: AbortSignal,
): Promise<MarketIndexSnapshot[]> {
  return fetchJson<MarketIndexSnapshot[]>(
...
export async function fetchLiveMarketDashboardData(
  ...
  let data = await fetchJson<MarketDashboardData>(
    "/api/korean-market/dashboard?fresh=1",
    signal,
    isLiveApiEnabled() ? API_BASE_URL : "",
  ...
  try {
    data = mergeMarketIndices(data, await fetchMarketIndicesData(signal));
  } catch {
    // The full dashboard is already live; keep it if the faster index refresh
    // fails due to rate limits or a transient KIS issue.
  }
