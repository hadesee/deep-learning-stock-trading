...
export function DashboardPage() {
  ...
  const [reuseCompletedAnalysis, setReuseCompletedAnalysis] = useState(true);
  ...
  async function handleRefreshDashboard() {
    ...
    setReuseCompletedAnalysis(false);
  ...
  async function handleRunCandidateAnalysis() {
    ...
    if (reuseCompletedAnalysis) {
      try {
        const status = await fetchCandidateAnalysisStatus();
        if (status.status === "completed" && status.result) {
          const newsNote = status.result.newsRows && status.result.newsRows > 0 ? ` · 뉴스 ${status.result.newsRows}종목` : "";
          const recovered = await revealResults(`최근 완료 분석 결과 — 후보 ${status.result.rows}개${newsNote}.`);
          if (recovered) {
        ...
        // If the status endpoint is unavailable, fall through and try a fresh run.
      }
    }
    ...
    setAnalysisErrorMessage(undefined);
    setReuseCompletedAnalysis(true);
